...
def _change_speed(sound_array: np.array, factor):
    if factor == 1:
...
def _stretch(sound_array, chunk, factor):
    hop = chunk / 7
    ...
    return np.int16(np.real(result))


def adjust_frequency(sound_array, frequency, expected_frequency, window_size=2 ** 11):
    factor = frequency / expected_frequency

    stretched = _stretch(sound_array=sound_array, chunk=window_size, factor=factor)
    modified_sound_array = _change_speed(stretched, 1 / factor)
    return modified_sound_array
